...
fn with_event_loop(
    native_options: &epi::NativeOptions,
    f: impl FnOnce(&mut EventLoop<RequestRepaintEvent>),
) {
    ...
    EVENT_LOOP.with(|event_loop| {
        ...
        if event_loop.is_none() {
            event_loop.replace(create_event_loop_builder(&native_options.clone()).build());
        }
...
mod glow_integration {
                    ...
                    if let Some(running) = &mut self.running {
                        match &event {
                            ...
                            winit::event::WindowEvent::CloseRequested
                                if running.integration.should_quit() =>
                            {
    ...
    pub fn run_glow(
        ...
        if native_options.run_and_return {
            with_event_loop(native_options, |event_loop| {
                let glow_eframe =
                    GlowWinitApp::new(event_loop, app_name, native_options, app_creator);
                run_and_return(event_loop, glow_eframe);
            });
        } else {
            let event_loop = create_event_loop_builder(native_options).build();
